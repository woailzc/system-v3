...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 类型中心 <span class="c-gray en">&gt;</span> 管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c">
	  <form action="<%=basePath%>a/pchPurchaseType/list.do" method="post">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><shiro:hasPermission name="pch:pchPurchaseType:del"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a></shiro:hasPermission><shiro:hasPermission name="pch:pchPurchaseType:save"> <a href="javascript:;" onclick="member_add('添加部门','<%=basePath%>a/pchPurchaseType/save.do?delFlag=1','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加</a></shiro:hasPermission></span> <span class="r">共有数据：<strong>${fn:length(pchPurchaseTypes)}</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		   <c:forEach items="${pchPurchaseTypes }" var="pchPurchaseType">
			<tr class="text-c">
				<td><input type="checkbox" value="${pchPurchaseType.id }" name="ids" id="ids"></td>
				<td>${pchPurchaseType.name}</td>
				<td><fmt:formatDate value="${pchPurchaseType.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>${pchPurchaseType.createBy.name}</td>
				<td class="td-manage"> 
				<shiro:hasPermission name="pch:pchPurchaseType:edit"><a title="编辑" href="javascript:;" onclick="member_edit('编辑','<%=basePath%>a/pchPurchaseType/update.do?id=${pchPurchaseType.id}&delFlag=1','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> </shiro:hasPermission>
				<shiro:hasPermission name="pch:pchPurchaseType:del"><a title="删除"  onClick="member_del(this,'${pchPurchaseType.id}')" href="javascript:;"class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></shiro:hasPermission>
				</td>
			</tr>
			</c:forEach>
		</tbody>
 ...
 function datadel(){
 	...
 	layer.confirm('确认要删除吗？',function(index){
 		$.ajax({
 			...
 			data:{ids:ids},
 			dataType: 'json',
 			url: '<%=basePath%>a/pchPurchaseType/dels.do',
 			success: function(data){
 				/* $(obj).parents("tr").remove(); */
 				layer.msg('已删除!',{icon:1,time:1000});
 				location.reload();
