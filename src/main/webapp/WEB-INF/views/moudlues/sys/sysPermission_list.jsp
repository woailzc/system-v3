...
<div class="page-container">
	<div class="text-c">
	  <form action="<%=basePath%>a/sysPermission/list.do" method="post">
		...
		<input type="text" class="input-text" style="width:250px" placeholder="输名" id="name" name="name">
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜搜索</button>
	   </form>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><shiro:hasPermission name="sys:sysPermission:del"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a></shiro:hasPermission><shiro:hasPermission name="sys:sysPermission:save"><a href="javascript:;" onclick="member_add('添加','<%=basePath%>a/sysPermission/save.do?delFlag=1','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限节点</a></shiro:hasPermission> </span> <span class="r">共有数据：<strong>${fn:length(sysPermissions)}</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		   <c:forEach items="${sysPermissions }" var="sysPermission">
			<tr class="text-c">
				<td><input type="checkbox" value="${sysPermission.id }" name="ids" id="ids"></td>
				<td>${sysPermission.sysMenu.menuName}</td>
				<td>${sysPermission.name}</td>
				<td>${sysPermission.remark}</td>
				<td><fmt:formatDate value="${sysPermission.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>${sysPermission.createBy.name}</td>
				<td class="td-manage"> 
				 <shiro:hasPermission name="sys:sysPermission:edit"><a title="编辑" href="javascript:;" onclick="member_edit('编辑','<%=basePath%>a/sysPermission/update.do?id=${sysPermission.id}&delFlag=1','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a></shiro:hasPermission>
				 <shiro:hasPermission name="sys:sysPermission:del"><a title="删除"  onClick="member_del(this,'${sysPermission.id}')" href="javascript:;"class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></shiro:hasPermission>
				 </td>
			</tr>
			</c:forEach>
		</tbody>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			data:{id:id},
			url: '<%=basePath%>a/sysPermission/del.do',
			dataType: 'json',
			success: function(data){
