...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><shiro:hasPermission name="sys:sysDepartment:del"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a></shiro:hasPermission><shiro:hasPermission name="sys:sysDepartment:save"> <a href="javascript:;" onclick="member_add('添加部门','<%=basePath%>a/sysDepartment/save.do?delFlag=1','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加部门</a></shiro:hasPermission></span> <span class="r">共有数据：<strong>${fn:length(sysDepartments)}</strong> 条</span> </div>
	<div class="mt-20">
...
function datadel(){
	...
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			...
			data:{ids:ids},
			dataType: 'json',
			url: '<%=basePath%>a/sysDepartment/dels.do',
			success: function(data){
				/* $(obj).parents("tr").remove(); */
