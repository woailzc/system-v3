...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><shiro:hasPermission name="ctn:ctnMenu:del"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a></shiro:hasPermission> <shiro:hasPermission name="ctn:ctnMenu:save"><a href="javascript:;" onclick="member_add('发布菜单','<%=basePath%>a/ctnMenu/save.do?delFlag=1','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 发布菜单</a></shiro:hasPermission></span> <span class="r">共有数据：<strong>${fn:length(ctnMenus)}</strong> 条</span> </div>
	<div class="mt-20">
...
function datadel(){
	...
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			...
			data:{ids:ids},
			dataType: 'json',
			url: '<%=basePath%>a/ctnMenu/dels.do',
			success: function(data){
				/* $(obj).parents("tr").remove(); */
				layer.msg('已删除!',{icon:1,time:1000});
				location.reload();
